...
class HandshakeAPITest {
    ...
    fun `test performHandshake with null-like string`() {
        // "null" as a string is NOT blank, so it should be accepted by validation
        // This test verifies we only check for blank, not semantic validity
        
        val nullString = "null"
        
        // Verify "null" string is not blank
        assertFalse(nullString.isBlank())
        
        // This would pass validation (though it's not a real public key)
        // We only validate blank-ness, not semantic correctness
        // Semantic validation would happen on the server side
        
        println("✅ String 'null' is not blank and passes client-side validation (expected)")
    }
    ...
    fun `test different types of whitespace are detected`() {
        ...
        whitespaceVariants.forEach { whitespace ->
            assertFailsWith<IllegalArgumentException> {
                url.performHandshake(whitespace)
            }
        }
